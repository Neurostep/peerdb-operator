...
func UninstallCertManager() {
	...
	cmd := exec.CommandContext(ctx, "kubectl", "delete", "-f", url) //nolint:gosec
...
func InstallCertManager() error {
	ctx := context.Background()
	url := fmt.Sprintf(certmanagerURLTmpl, certmanagerVersion)
	cmd := exec.CommandContext(ctx, "kubectl", "apply", "-f", url) //nolint:gosec
	if _, err := Run(cmd); err != nil {
		return err
	}
...
func LoadImageToKindClusterWithName(name string) error {
	...
	cmd := exec.CommandContext(context.Background(), kindBinary, kindOptions...) //nolint:gosec
	_, err := Run(cmd)
	return err
}
