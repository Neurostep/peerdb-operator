...
// PeerDBWorkerPoolReconciler reconciles a PeerDBWorkerPool object
...
// +kubebuilder:rbac:groups=peerdb.peerdb.io,resources=peerdbworkerpools/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=peerdb.peerdb.io,resources=peerdbworkerpools/finalizers,verbs=update
// +kubebuilder:rbac:groups=apps,resources=deployments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups="",resources=events,verbs=create;patch

func (r *PeerDBWorkerPoolReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Build and reconcile the Flow Worker Deployment.
	desired := resources.BuildFlowWorkerDeployment(pool, cluster, configHash)
	...
	// Update if template or replicas changed (skip during backup fencing).
	if !backupInProgress {
		needsUpdate := false
		...
		autoscalingEnabled := pool.Spec.Autoscaling != nil && pool.Spec.Autoscaling.Enabled
		if !autoscalingEnabled && !equality.Semantic.DeepEqual(existing.Spec.Replicas, desired.Spec.Replicas) {
	...
	pool.Status.ObservedGeneration = pool.Generation
	peerdbmetrics.WorkerPoolsTotal.WithLabelValues(pool.Name, pool.Namespace).Set(float64(pool.Status.Replicas))

	ready := existing.Status.ReadyReplicas > 0 && existing.Status.ReadyReplicas == existing.Status.Replicas
	if ready {
		meta.SetStatusCondition(&pool.Status.Conditions, metav1.Condition{
			Type:               peerdbv1alpha1.ConditionReady,
		...
		meta.SetStatusCondition(&pool.Status.Conditions, metav1.Condition{
			...
			Status:             metav1.ConditionFalse,
			ObservedGeneration: pool.Generation,
			Reason:             peerdbv1alpha1.ReasonDeploymentNotReady,
			Message:            fmt.Sprintf("%d/%d replicas are ready", existing.Status.ReadyReplicas, existing.Status.Replicas),
		})
	}
...
func (r *PeerDBWorkerPoolReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&peerdbv1alpha1.PeerDBWorkerPool{}).
